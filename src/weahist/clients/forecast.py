...
Used to cover the trailing few days that the archive endpoint hasn't
finalized yet (it lags ~1\u20133 days). The forecast endpoint covers today
plus a configurable window into the past via `start_date`/`end_date`.
The response shape matches the archive endpoint, so the merged frames
are interchangeable.
"""
...
class ForecastClient:
    ...
    def fetch(
        ...
        """Return a UTC-indexed DataFrame of forecast/recent observations."""
        ...
        data = self._http.get_json(self._settings.forecast_url, params)
        block = data.get(block_key)
        if not isinstance(block, dict) or "time" not in block:
            raise UpstreamError(f"forecast response missing {block_key!r} block")
